                ...
                <div ref="navWrap" :class="[prefixCls + '-nav-wrap', scrollable ? prefixCls + '-nav-scrollable' : '']">
                    ...
                    <div ref="navScroll" :class="[prefixCls + '-nav-scroll']" @DOMMouseScroll="handleScroll" @mousewheel="handleScroll">
                        <div ref="nav" :class="[prefixCls + '-nav']" :style="navStyle">
                            ...
                            <div :class="tabCls(item)" :key="index" v-for="(item, index) in navList" @click="handleChange(index)">
                                <Icon v-if="item.icon !== ''" :size="item.iconSize" :type="item.icon"></Icon>
...
<script>
    // import Icon from 'view-design/src/componentsicon/icon.vue';
    import Render from 'view-design/src/components/base/render';
    import { oneOf, MutationObserver, findComponentsDownward } from 'view-design/src/utils/assist';
        ...
        methods: {
            getTabs () {
                ...
                AllTabPanes.forEach(item => {
                    if(item.uniqueKey!=null && item.uniqueKey!=''){
            ...
            updateNav () {
                ...
                this.getTabs().forEach((pane, index) => {
                    this.navList.push({
                        ...
                        isFirstOne: pane.isFirstOne, //add by andy
                        uniqueKey:pane.uniqueKey || (pane.currentName || index), //add by andy
            ...
            handleChange (index) {
               // debugger
                ...
                //edit by andy ,返回对象,增加唯一键
                let params = {
                    name:nav.name,
                    uniqueKey:nav.uniqueKey
                }
            ...
            handleRemove (_index) {
                //debugger
                //console.log('=====tab handleRemove======='+_index)
                let index = _index
            ...
            handleRemoveTab (index) {
                ...
                if(index===0){
                     ...
                     this.$emit('on-tab-remove', params);
                    return
                }
               // debugger
                const tabs = this.getTabs();
            ...
            showClose (item) {
                //debugger
                if (this.type === 'card') {
                    if(item.isFirstOne){  //屏蔽第一个,不可以关闭
