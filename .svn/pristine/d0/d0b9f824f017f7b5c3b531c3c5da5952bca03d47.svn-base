  ...
  <div class="cl-moudle cl-analyzerNew">
    ...
    <div slot="top">
      <Tabs @on-click="TabsClickEvent" :animated="false">
        <TabPane label="客户汇总" name="detailed">
          <vTable
            :height="tableHeight-26"
            ref="master_list_table"
            :columns-url="functionParams.requestColBaseUrl + '/statisAccCustCalGroupFm'"
            :table-data="tableBoxCoWorkProcData"
            :pagination="false"
            name="statisAccCustCalGroupFm"
          ></vTable>
        </TabPane>

        <TabPane label="业务员汇总" name="Summary">
          <vTable
            :height="tableHeight-26"
            ref="tableFieldRef"
            :table-data="tableBoxCoWorkProcData"
            :columns-url="functionParams.requestColBaseUrl + '/statisAccWorkerCalGroupFm'"
            :pagination="false"
            name="statisAccWorkerCalGroupFm"
          ></vTable>
        </TabPane>
        <ReportPage 
            slot="extra" 
...
export default {
  ...
  methods: {
    ...
    getreportUrl(){
      let url = '/spGetAccCustCalGroup'
      if (this.currentTabName=="Summary") {
        url = '/spGetAccWorkerCalGroup'
      }
      return url
    },
    // tab 切换事件
    ...
    LoadBoxCoWorkProcData(url) {
        ...
        let _self = this
        request.post(`/report/receiCalAccount${url}`,params,{pageNumber:this.page.pageNumber,pageSize:this.page.pageSize}).then(res=>{
  ...
  created() {
    this.LoadBoxCoWorkProcData('/spGetAccCustCalGroup');
  },
  mounted() {}
};
</script>
