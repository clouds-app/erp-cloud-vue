    ...
    >
      <div  @contextmenu.prevent ref="masterHeight" class="masterHeightClass" :style="{ height: getMasterheight() + '%' }" >
        ...
        >
            ...
            <Col span="6">
                <FormItem  label="送货日期" prop="bdDate">
                   <DatePicker
                    ...
                    transfer
                    :value="formDataInfo.master.bdDate"
                    type="datetime"
            ...
            <Col span="6">
                <FormItem label="客户编号" prop="custCode">
                  <popup
                    ...
                    render-fields="custId,custCode,custName,coinId,areaIds,areaNames,addrDetail,bdSaleId,bdSaleCode,bdSaleName,followerId,followerCode,followerName"
             ...
             <Col span="6">
                <FormItem label="车牌" prop="bdCarNo">
                  <popup
                        ...
                        :fill-model.sync="formDataInfo.master"
                        render-fields="bdCarNo,carDriverId,carDriverCode,carDriverName"
            ...
            <Col span="6">
              ...
              <Col span="6">
                <FormItem :label-width="90" label="月结日期" prop="bdMonthDate">
                   <DatePicker
                    @on-change="onChange_bdMonthDate"
                    transfer
                    :value="formDataInfo.master.bdMonthDate"
                    type="datetime"
                    format="yyyy-MM-dd HH:mm:ss"
                  ></DatePicker>
                </FormItem>
            ...
            <Col span="6">
                <FormItem label="送货地址" prop="areaNames">
                     <popup  :beforeOnFillValidator="beforeOnFillValidator"   :popupClickValidator="popupClickValidator" v-model="formDataInfo.master.areaNames" field-name="areaNames" :disabled="false" popup-name="AddrDetailSingleBox"
            ...
            <Col span="6">
                <FormItem label="详细地址" prop="addrDetail">
                   <popup   :beforeOnFillValidator="beforeOnFillValidator" :popupClickValidator="popupClickValidator" v-model="formDataInfo.master.addrDetail" field-name="addrDetail" :disabled="false" popup-name="AddrDetailSingleBox"
                      :fill-model.sync="formDataInfo.master" render-fields="areaIds,areaNames,addrDetail" from-fields="areaIds,areaNames,addrDetail"
                      :query-params="{custId:formDataInfo.master.custId}"
                      :blur-focus-load-data="false"
                      />
        ...
        <Tabs :animated="false" :value="currentTabName">
            <TabPane label="纸箱出货明细" name="stockboxdeliitem">
                ...
                :data.sync="formDataInfo['boxDeliItems'].defaultList"
             ...
             <template  v-for="(column,subIndex) in initData.columns[`${functionParams.formInitPreName}itemFm`].editColumns">
                   <td :key="subIndex" :class="`ivu-table-column-${column.align}`" v-if="excludeFiled('itemFm',column.key)" :width="column.editWidth">
                    ...
                    <Select @on-change="value => {valueChangeAssign(value, index, row,column.key)}" :disabled="column.readOnly" v-else-if="column.key == 'unitType'" v-model="row[column.key]" transfer>
                     ...
                     <!-- 少数-->
                     <template  v-else-if="column.key ==='biLessQty'">
                      <formControl  @input="tableColTiggerEvent(row,column,index,'biLessQtyChangeEvent')"  :control-type="column.controlType" v-model="row[column.key]"></formControl>
                    </template>
                      <!-- 单价-->
                     <template  v-else-if="column.key ==='biPrice'">
                      <formControl  @input="tableColTiggerEvent(row,column,index,'biPriceChangeEvent')"  :control-type="column.controlType" v-model="row[column.key]"></formControl>
                    </template>
                     <!-- 新单价-->
                     <template  v-else-if="column.key ==='biNewPrice'">
                      <formControl  @input="tableColTiggerEvent(row,column,index,'biNewPriceChangeEvent')"  :control-type="column.controlType" v-model="row[column.key]"></formControl>
                    </template>
            ...
            <TabPane label="出货其他费用" name="boxDeliOtherFee">
                ...
                :data.sync="formDataInfo['boxDeliOtherFee'].defaultList"
             ...
             <template  v-for="(column,subIndex) in initData.columns[`${functionParams.formInitPreName}otherfeeFm`].editColumns">
                   ...
                   <Select @on-change="value => {valueChangeAssign(value, index, row,column.key)}" :disabled="column.readOnly||detailDisabled" v-else-if="column.key == 'priceType'" v-model="row[column.key]" transfer>
                    ...
                    <template  v-else-if="column.key ==='unitPrice'">
                      <formControl  @input="tableColTiggerEvent(row,column,index,'unitPriceChangeEvent')"  :control-type="column.controlType" v-model="row[column.key]"></formControl>
                    </template>
                      <template  v-else-if="column.key ==='qty'">
                      <formControl  @input="tableColTiggerEvent(row,column,index,'qtyChangeEvent')"  :control-type="column.controlType" v-model="row[column.key]"></formControl>
                    </template>
                    <template v-else>
                        <formControl :align="column.align" :control-type="column.controlType"
                          v-model="row[column.key]"
...
const default_formDataInfo = {
  ...
  master: {
    ...
    bdCarNo: '',
    bdCube: null,
    bdDate: new Date(dayjs().format('YYYY-MM-DD HH:mm:ss')), // 送货日期
    areaNames: '',
    bdDecAmount: null,
    bdDecReason: '',
    bdFeer: null,
    ...
    biQty: null, // 数量
    bdMoney: null,
    bdMonthDate: new Date(dayjs().format('YYYY-MM-DD HH:mm:ss')), // 月结日期
    bdNo: '',
    bdSaleCode: '',
    bdSaleId: '',
    bdSaleName: '',
...
export default {
  ...
  data () {
    return {
      currentTabName: 'stockboxdeliitem',
      ...
      masterRuleValidate: {
        bdDate: [{ required: true, type: 'date', message: '送货日期不能为空', trigger: 'blur' }],
        custCode: [{ required: true, message: '客户编号不能为空', trigger: 'blur' }],
        bdMonthDate: [{ required: true, type: 'date', message: '月结日期不能为空', trigger: 'blur' }]
      }, // 纸箱出货 需要验证的数据
      ...
      feeTypeList: [], // 费用类型列表
      bpUnitList: [], // 开单单位列表
      selectedWorkNoList: [], // 已经选择的工单列表ID
  ...
  methods: {
    onChange_bdBackDate (item) {
      if (!_.isEmpty(item)) {
        this.formDataInfo.master.bdDate = new Date(item)
      }
    },
    onChange_bdMonthDate (item) {
      if (!_.isEmpty(item)) {
        this.formDataInfo.master.bdMonthDate = new Date(item)
      }
    },
    ...
    tableColTiggerEventCall (obj) {
      switch (obj.event) {
        ...
        case 'qtyChangeEvent':
          let currentItem = this.formDataInfo['boxDeliOtherFee'].defaultList[obj.index]
          let crrentPrice = 0
          if (currentItem.unitPrice != null || currentItem.unitPrice != '') {
            crrentPrice = Number(currentItem.unitPrice)
          }
          let totalMoney = crrentPrice * Number(currentItem.qty == null ? 0 : currentItem.qty)
          // 触发修改值 直接修改原始数据 无效
          this.$refs['sub_list_table_edit'].set({ unitPrice: obj.row.unitPrice, qty: obj.row.qty, money: Number(totalMoney).toFixed(2) }, obj.index)
          break
        default:
          break
      }
    ...
    tableColTiggerEventSlaveCall (obj) {
      let currentItem = this.formDataInfo['boxDeliItems'].defaultList[obj.index]
      let totalMoney = 0, crrentPrice = 0
      let biQty = Number(currentItem.biQty == null ? 0 : currentItem.biQty) // 送货数量
      let biLessQty = Number(currentItem.biLessQty == null ? 0 : currentItem.biLessQty) // 少数
      if (currentItem.biNewPrice == null || currentItem.biNewPrice == '' || currentItem.biNewPrice == '0' || currentItem.biNewPrice == 0) {
      ...
      switch (obj.event) {
        ...
        case 'biLessQtyChangeEvent':
          ...
          let leftQty = (biQty - biLessQty)
          let biWeight = Number((leftQty * Number(currentItem.biSWeight)).toFixed(toFixedUnit))// 重量 = (送货数量-少数)*单重
          let biCube = Number((leftQty * Number(currentItem.biSCube)).toFixed(toFixedUnit))// 体积 = (送货数量-少数)*单体积
          let biArea = Number((leftQty * Number(currentItem.biSArea)).toFixed(toFixedUnit))// 面积 = (送货数量-少数)*单面积
          this.$refs['slave_list_table_edit'].set({ biWeight: Number(biWeight).toFixed(2), biCube: Number(biCube).toFixed(2), biArea: Number(biArea).toFixed(2) }, obj.index)
          break
          // 单价 变更
      ...
      totalMoney = crrentPrice * (biQty - biLessQty)
      // 触发修改值 直接修改原始数据 无效
      this.$refs['slave_list_table_edit'].set({ biMoney: Number(totalMoney).toFixed(2) }, obj.index)
      this.sumMoneyAreaVolumeWeight()
    },
    // 设置默认值
    setDefaultData () {
      this.formDataInfo.master.bdDate = new Date(dayjs(this.formDataInfo.master.bdDate).format('YYYY-MM-DD HH:mm:ss'))
      this.formDataInfo.master.bdMonthDate = new Date(dayjs(this.formDataInfo.master.bdMonthDate).format('YYYY-MM-DD HH:mm:ss'))
      let otherFeeFm = this.formDataInfo['boxDeliOtherFee'].defaultList[0]
      if (otherFeeFm && otherFeeFm.feeTypeText == '') {
        otherFeeFm.feeType = '1'
        otherFeeFm.feeTypeText = '1:客户自提优惠'
      }
      if (otherFeeFm && otherFeeFm.priceTypeText == '') {
        otherFeeFm.priceType = '1'
        otherFeeFm.priceTypeText = '1:量价'
      }
    },
    ...
    getbpUnitList () {
      // 获取  开单单位
      this.getDataFromDictionaryBy('bpUnit').then(res => {
        this.bpUnitList = res
      })
    },
    getfeeTypeList () {
    ...
    beforeOnFillValidator (msg = '') {
      ...
      return new Promise((resolve, reject) => {
        let curDataList = this.formDataInfo['boxDeliItems'].defaultList
        if (curDataList && curDataList[0].workNo == '') {
          // fix 添加时,如果已经有明细数据,修改地址 也该清空数据 并且提示
          reject()
          return
        }
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            _self.formDataInfo['boxDeliItems'].defaultList = []
    ...
    onSubmitEditForm (dataList) {
      ...
      if (dataList && Array.isArray(dataList) && dataList.length > 0) {
        ...
        if (boxDeliItems[0].bcNo == '') {
          ...
          // 删除多余的空行
          this.formDataInfo['boxDeliItems'].defaultList.forEach((item, index) => {
            if (item.workNo == '' || item.workNo == null) {
              this.formDataInfo['boxDeliItems'].defaultList.splice(index, 1)
            }
          })
        }
        this.sumMoneyAreaVolumeWeight()
    ...
    // 检查列表数据是否已经存在,避免重复添加
    checkIsExistBy (item) {
      let currentList = this.formDataInfo['boxDeliItems'].defaultList
      let isExistIndex = currentList.findIndex(subItem => {
        return (subItem.workNo == item.workNo && subItem.bcNo == item.bcNo && subItem.biProdNo == item.biProdNo && subItem.bdStationName == item.bdStationName)
      })
      if (isExistIndex != -1) {
        return true
      } else {
    ...
    sumMoneyAreaVolumeWeight () {
      ...
      this.formDataInfo.master.bdMoney = (totalMoney + Number(otherTotalFee)).toFixed(toFixedUnit) // 金额
    ...
    sumOtherFee () {
      let totalMoney = 0
      this.formDataInfo['boxDeliOtherFee'].defaultList.forEach(item => {
        if (!!item.workNo && !!item.feeName) {
    ...
    openProductsList () {
      ...
      if (this.formDataInfo['boxDeliItems'] && this.formDataInfo['boxDeliItems'].defaultList.length > 0) {
        this.formDataInfo['boxDeliItems'].defaultList.forEach(item => {
          if (this.selectedWorkNoList == '') {
            this.selectedWorkNoList = item.workNo
          } else {
            this.selectedWorkNoList += ',' + item.workNo
          }
        })
