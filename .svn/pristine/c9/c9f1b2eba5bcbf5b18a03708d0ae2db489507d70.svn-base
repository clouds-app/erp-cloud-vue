...
export default {
  ...
  methods: {
    ...
    master_list_tableRowClick (rowData, rowIndex) {
      ...
      this.$refs['tableFieldRef'].search({ brId: rowData.id })
    },
    // 纸箱出货明细 行点击事件
    ...
    excludeFiled (type, key) {
      ...
      let exListBrDate = ['returnDate'] // 月结时 batchUpdateMonthDate
      let exListMonthDate = ['brDate'] // 退货时  需要隐藏字段
      let exList = []
      switch (type) {
