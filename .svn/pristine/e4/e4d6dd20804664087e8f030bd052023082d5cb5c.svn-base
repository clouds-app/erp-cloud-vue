          ...
          <Col span="6">
            <FormItem label="客户编号" prop="custCode">
                  <popup 
                    ...
                    render-fields="custId,custCode,custName,curyId,coinCode,coinName,taxRat,workName"
                    from-fields="id,cusCode,cusName,coinId,coinCode,coinName,taxRate,saleName"
                    :suffix="true"
  ...
  methods: {
    ...
    setDefaultData(){
        ...
        if(this.formDataInfo.omDate==''|| this.formDataInfo.omDate==null){
            this.formDataInfo.omDate = new Date(dayjs().format("YYYY-MM-DD HH:mm:ss"))
        }else{
            this.formDataInfo.omDate = new Date(dayjs(this.formDataInfo.omDate).format("YYYY-MM-DD HH:mm:ss"))
        }

        if(this.formDataInfo.omYm==''|| this.formDataInfo.omYm==null){
            this.formDataInfo.omYm = new Date(dayjs().format("YYYY-MM"))
        }else{
            this.formDataInfo.omYm = new Date(dayjs(this.formDataInfo.omYm).format("YYYY-MM"))
        }
    },
    //单据日期 变动日期
