      ...
      <Form :label-width="80" ref="setGetBackWindow" :model="formGetBack" :rules="ruleGetBackWindow">
            ...
            <FormItem label="客户名称" prop="custName">
                 <popup
                    ...
                    render-fields="custId,custCode,custName,areaIds,areaNames,addrDetail,bdSaleId,bdSaleCode,bdSaleName,followerId,followerCode,followerName"
  ...
  methods: {
    ...
    setEndDate () {
      ...
      let choseDataList = this.tableEndDateDataList.forEach(item => {
        if (item.iisChose) {
          if (!this.formEndDataInfo.biIsAcc) { // 不采用设定日期
