...
export default {
  ...
  data() {
    return {
      accPayItemBillTypeList: [], // 应收对账单明细单据类型
      // 查询配置参数
      functionParams: {
        formInitPreName: "accreceitembox", // 查询表格列头信息 前缀 例如:saleboxproductprice Fm/Fm/mdataFm
  ...
  methods: {
    ...
    searchDataBy() {
      ...
      let params = {
        ...
        signBack: this.searchParams.signBack,//signBack(需回签)
        custId: this.searchParams.custId, //(供应商id)
      ...
      request
        .post(`${this.functionParams.requestBaseUrl}/spAutoAccRece`, params)
        .then(res => {
          if (res && res.length > 0) {
            this.formDataInfo["accReceItems"].defaultList = res;
          }
    ...
    resetformDataInfo() {
      //debugger
      let choseDataList = this.formDataInfo["accReceItems"].defaultList.filter(
        item => {
