...
<script>
/**
 * @desc  edit-vendInvoiceCheckSlave 描述 收票选单据
...
export default {
  ...
  data() {
    return {
        // 查询配置参数
      functionParams: {
        formInitPreName: 'accountvicitembillnobox', // 查询表格列头信息 前缀 例如:saleboxproductprice Fm/Fm/mdataFm
  ...
  methods: {
      ...
      searchDataBy(){
       ...
       //参数包括inShortName 
      //startDate(开始日期),endDate(结束日期),supplierId(供应商id),likeFlag(1模糊查询0精准查询),billNo(单据编号),billNoList(已选的单据编号集合，用逗号隔开)
        let params = {
          ...
          billNo:this.getCurrentKeyTypeWords('billNo'),//(单据号)
          inShortName:this.getCurrentKeyTypeWords('shortName'),//(客户简称)
          likeFlag:this.formDataInfo.master.likeFlag,//(1模糊查询0精准查询)
          billNoList:this.searchParams.billNoList//过滤参数
        }
        this.loadingData=true
         request.post(`${this.functionParams.requestBaseUrl}/getInvoiceBoxData`, params).then(res => {
          if(res && res.length>0){
            this.formDataInfo['vicItems'].defaultList =res
          }
      ...
      loadColumsData(){
        ...
        //debugger
          let _self = this
          let formName="accountvicitembillnoboxFm"
      ...
      excludeFiled(type,key){
        ...
        //billNo 单据号，shortName 简称
        let searchList = ['billNo','shortName']
        let exList=[]
        switch (type) {
            case 'itemFm':
              exList = exListItemFm 
    ...
    resetformDataInfo() {
        //debugger
      let choseDataList = this.formDataInfo['vicItems'].defaultList.filter(item=>{
...
<style>
.cl-edit-window-vendInvoiceCheck .masterHeightClass{
  height: 50%;overflow:auto
}
.cl-edit-window-vendInvoiceCheck .otherHeightClass{
