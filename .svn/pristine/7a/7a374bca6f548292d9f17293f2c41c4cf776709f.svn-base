  ...
  <div>
    ...
    >
      ...
      >
        <Row>
          <Col span="6">
            <FormItem label="外发单号" prop="sendNo">
              <Input disabled v-model="formDataInfo.master.sendNo" maxlength="20" placeholder></Input>
            </FormItem>
          </Col>
          <Col span="6">
            <FormItem label="外发日期" prop="sendDate">
              <DatePicker
                type="date"
                format="yyyy-MM-dd HH:mm:ss"
                v-model="formDataInfo.master.sendDate"
              ></DatePicker>
            </FormItem>
          </Col>
          <Col span="6">
              <FormItem label="交货日期" prop="delDate">
                <DatePicker
                  type="date"
                  format="yyyy-MM-dd HH:mm:ss"
                  v-model="formDataInfo.master.delDate"
                ></DatePicker>
              </FormItem>
            </Col>
            <Col span="6">
      ...
      <Tabs>
        ...
        <TabPane label="纸箱工序外发明细" name="name1">
        <!-- <TabPane :label="initData.initData.mrpWproSendOutItemFm" name="name1"> -->
          <eTable
            ref="tableFields"
            unqiue-mark="id"
            :index-menu="true"
            ...
            :row-init-data="initData.initData.mrpWproSendOutItemFm"
            :data.sync="formDataInfo.proSendOutItemSlaves.defaultList"
            :rules="tableFieldsValidator"
            :showContextMenu="!detailDisabled"
          >
            <template slot="head">
              <tr
                v-for="(columnGroup,index) in initData.columns.mrpWproSendOutItemFm.editGroups"
                :key="index"
              >
                <th
            ...
            <template slot="body" slot-scope="{ row, index, valueChangeAssign }">
              <td
                ...
                v-for="(column,columnIndex) in initData.columns.mrpWproSendOutItemFm.editColumns"
                :key="columnIndex"
                :width="column.editWidth"
              >
                 <!--控件特殊处理 工单号  -->
...
import Slave from './edit-procedureoutSlabe'
const default_formDataInfo = {
  ...
  master: {
    ...
    delDate: dayjs().format('YYYY-MM-DD HH:mm:ss'),
    loadorId: '',
    loadorCode: '',
    loadorName: '',
    sendType: '0',
...
export default {
  ...
  data () {
    return {
      ...
      List: null,
      index: 0, // 存储点击弹框行号
      actionSubtitle: '纸箱工序外发', // 当前操作副标题
      formName: 'mrpWproSendOutItemFm',
      requestBaseUrl: '/mrp/wproSendOut', // 请求 查询 操作的基础路径
  ...
  methods: {
    ...
    getamt (index) {
      let data = this.$refs.tableFields.get()
      let qty = Number(data[index].qty)// 当前行数量
      let prepQty = Number(data[index].prepQty)// 当前行备品数
      let unitPrice = Number(data[index].unitPrice)// 当前行单价
    ...
    resetformDataInfo (_data) {
      if (this.formDataInfo.master.delDate) {
        this.formDataInfo.master.delDate = dayjs(this.formDataInfo.master.delDate).format(
          'YYYY-MM-DD HH:mm:ss'
        )
        // return this.formDataInfo;
      }
      if (this.formDataInfo.master.sendDate) {
        this.formDataInfo.master.sendDate = dayjs(this.formDataInfo.master.sendDate).format(
          'YYYY-MM-DD HH:mm:ss'
        )
      }
      this.formDataInfo.proSendOutItemSlaves = this.$refs['tableFields'].getCategorizeData()
      return this.formDataInfo
    },
    // 打开编辑框调用
    openActionTigger () {
