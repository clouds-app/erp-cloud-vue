  ...
  <div class="functionBtnListContent" style="display: flex;">
    ...
    <Row>
      ...
      <Col  span="24">
        <Row>
          <Col  span="8">开始
           <Select @on-change="printStartRowOnchange" v-model="printStartRow" style="width:120px">
               <Option value="1" key="1">从第一行开始</Option>
               <Option value="2" key="2">当前位置开始</Option>
          </Select>
          </Col>
           <Col  span="8">结束
            <Select @on-change="printEndRowOnchange" v-model="printEndRow" style="width:120px">
              <Option value="1" key="1">到最后行结束</Option>
              <Option value="2" key="2">当前位置结束</Option>
              <Option value="3" key="3">指定列印行数</Option>
          </Select>
          </Col>
            <Col  span="4">
              <InputNumber @on-change="mutilplePrintFunctionOn" :disabled="printEndRow!=='3'" v-model="printSpRow" :min="1" style="width:60px" size="small"></InputNumber>
            </Col>
        </Row>
      </Col>
    </Row>
...
export default {
  ...
  methods: {
    ...
    printTableCancle () {
      ...
      this.printStartRow = '2'// 开始列印行数
      this.printEndRow = '2'// 结束列印行数
    },
    // 开始列印行数 选择改变触发事件
    printStartRowOnchange (value) {
      this.mutilplePrintFunctionOn()
    },
    // 结束列印行数 选择改变触发事件
    printEndRowOnchange (value) {
      if (value !== '3') {
        this.printSpRow = '1'// 指定列印行数
      }
      this.mutilplePrintFunctionOn()
    ...
    // 多行打印功能启动
    mutilplePrintFunctionOn () {
      // 打印模板参数
      let seletedItem = this.printTableDataList.filter(item => {
        return item.iisChose == true
      })
      if (seletedItem && seletedItem.length < 1) {
        return
      }
      let selectionItem = this._install.getMasterSelectItem() // 当前选择行
      let tableDataItems = this._install.getTableDataItems() // 当前主表数据列表
      let printTemplateTableParams = seletedItem[0]

      let currentSeletedItemIndex = tableDataItems.findIndex(item => { // 当前选择行的数组对应的下标
        return item.id == selectionItem.id
      })
      let sliceStartIndex = 0 // 截取数据开始下标
      let sliceEndIndex = 0 // 截取数据下标结束下标
      let lastArrayIndex = (tableDataItems.length) // 最后一行下标
      switch (this.printStartRow) {
        case '1': // 第一行开始
          sliceStartIndex = 0
          currentSeletedItemIndex = currentSeletedItemIndex + 1
          break
        case '2': // 当前选择行
          sliceStartIndex = currentSeletedItemIndex
          break
      }
      switch (this.printEndRow) {
        case '1': // 结尾
          sliceEndIndex = lastArrayIndex
          break
        case '2': // 当前
          sliceEndIndex = currentSeletedItemIndex
          break
        case '3': // 指定
          if (this.printSpRow > lastArrayIndex) {
            sliceEndIndex = lastArrayIndex
          } else {
            sliceEndIndex = this.printSpRow
          }
          break
      }
      let paramsStrList = ''
      // 截取指定 下标 列表
      let DataListAfterFilter = tableDataItems.slice(sliceStartIndex, sliceEndIndex)
      if (DataListAfterFilter && DataListAfterFilter.length == 0) {
        this.formatPrintUrl(seletedItem[0])
        return
      }
      // 打印模板参数 是否多个
      if (printTemplateTableParams.paramsName.includes(',')) {
        // 多个参数
        let paramList = printTemplateTableParams.paramsName.split(',')
        paramList.forEach(item => {
          if (paramsStrList == '') {
            let configSetting = ''
            DataListAfterFilter.forEach(subItem => {
              if (configSetting == '') {
                configSetting = `${subItem[item]}`
              } else {
                configSetting += `,${subItem[item]}`
              }
            })
            paramsStrList = `${item}=` + configSetting
          } else {
            let configSetting = ''
            DataListAfterFilter.forEach(subItem => {
              if (configSetting == '') {
            ...
            paramsStrList += `&${item}=` + configSetting
            // id=1269110474285342721,1257841943933693954&supplierCode=A13,A12
          }
        })
        this.printTableConfigStr = `templateId=${printTemplateTableParams.id}&${paramsStrList}`
      } else {
        // 当个参数
        DataListAfterFilter.forEach(subItem => {
          if (paramsStrList == '') {
            paramsStrList = subItem.id
          } else {
            paramsStrList = paramsStrList + ',' + subItem.id
          }
        })
        this.printTableConfigStr = `templateId=${printTemplateTableParams.id}&${printTemplateTableParams.paramsName}=${paramsStrList}`
      }
    },
    // 是否拥有此扩展功能
