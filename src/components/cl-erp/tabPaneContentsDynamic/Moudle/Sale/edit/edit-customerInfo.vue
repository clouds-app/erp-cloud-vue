<template>
  <div>
    ...
    >
    <Row>
        <Tabs :animated="false" value="baseInfo">
      ...
      >
        ...
        <TabPane label="财务对账" name="finance">财务对账</TabPane>
        <TabPane label="送货信息" name="name3">送货信息</TabPane>
          <TabPane label="信用额与重量" name="Delivery">信用额与重量</TabPane>
            <TabPane label="计价设定" name="Valuation">计价设定</TabPane>
              <TabPane label="公关/负责人信息" name="PR">公关/负责人信息</TabPane>
                <TabPane label="合同/其他详细信息" name="contract ">合同/其他详细信息</TabPane>
                  <TabPane label="单据提示/质量要求" name="requirements">单据提示/质量要求</TabPane>
        </Tabs>
    </Row>

      <Tabs>
        <!--  注意:eTable formDataInfo.wareHouseItems.defaultList  ===wareHouseItems=== 需要根据实际接口修改,其它不变-->
        <TabPane label="送货地址" name="name1">
          <eTable
            ref="tableFields"
            unqiue-mark="id"
            :height="400"
