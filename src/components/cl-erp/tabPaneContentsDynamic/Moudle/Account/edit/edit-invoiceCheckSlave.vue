...
<script>
/**
 * @desc  edit-invoiceCheckSlave 描述 开票选单据
...
export default {
  ...
  data() {
    return {
        // 查询配置参数
      functionParams: {
        formInitPreName: 'invoiceCheckArNoDto', // 查询表格列头信息 前缀 例如:saleboxproductprice Fm/Fm/mdataFm
  ...
  methods: {
      ...
      searchDataBy(){
       ...
       //参数包括 参数(inCustId 客户id，inArNo 单据号，inShortName 简称，inFlag 模糊查询，beginDate 起始日期，endDate 结束日期)
        ...
        this.loadingData=true
         request.post(`${this.functionParams.requestBaseUrl}/getArNoPopup`, params).then(res => {
          if(res && res.length>0){
            this.formDataInfo['invoiceCheckItems'].defaultList =res
          }
      ...
      loadColumsData(){
        //纸箱出货弹出框主表数据(虚拟)
          let _self = this
          let formName="invoiceCheckArNoDtoFm"
    ...
    // 重写父类,修改提交数据
    resetformDataInfo() {
      let choseDataList = this.formDataInfo['invoiceCheckItems'].defaultList.filter(item=>{
