      ...
      <Tabs @on-click="TabsClickEvent" :animated="false">
        <TabPane label="明细" name="detailed">
          <vTable
            ...
            :columns-url="functionParams.requestColBaseUrl + '/accountCustInitfm'"
            :table-data="tableBoxCoWorkProcData"
            :pagination="false"
            name="accountCustInitfm"
          ></vTable>
        </TabPane>
        ...
        <TabPane label="汇总" name="Summary">
          <vTable
            ...
            ref="tableFieldRef"
            :table-data="tableBoxCoWorkProcData"
            :columns-url="functionParams.requestColBaseUrl + '/statisAccountInitGroupFm'"
            :pagination="false"
            name="statisAccountInitGroupFm"
          ></vTable>
        </TabPane>
        <ReportPage 
            slot="extra" 
...
export default {
  ...
  data() {
    return {
      ...
      // 查询配置参数
      functionParams: {
        formInitPreName: "accountCustInitfm", // 查询表格列头信息 前缀 例如:boxco mainFm/itemFm/mdataFm
  ...
  methods: {
    ...
    getreportUrl(){
      let url = '/getInitDetailPage'
      if (this.currentTabName=="Summary") {
        url = '/getInitDetailGroupPage'
      }
      return url
    },
    // tab 切换事件
  ...
  created() {
    this.LoadBoxCoWorkProcData('/getInitDetailPage');
  },
  mounted() {}
};
</script>
