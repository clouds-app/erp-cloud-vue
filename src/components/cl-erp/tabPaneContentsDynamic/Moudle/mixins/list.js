  ...
  methods: {
    ...
    showDetailswindow(rowData, rowIndex, column){
      ...
      this.dbclikTimer = setTimeout(()=>{
        // 快速双击 ==>解决重复查询的问题
          let htmlTemplateObj = this.$refs['htmlTemplate']
          if(!!htmlTemplateObj && Object.keys(htmlTemplateObj).length>0){
              let innerFunctionBtnListObj = htmlTemplateObj.$refs['innerFunctionBtnList']
              innerFunctionBtnListObj.detailAction()
          }
      },500)
