...
export default {
  ...
  components: {
    editForm: function(resolve) {
      //组件的异步加载
      require(["./edit/edit-boxLibLost"], resolve);
    },
    htmlTemplate,
    vTable
  },
  data() {
    return {
      ...
      functionParams: {
        formInitPreName:'stockboxliblost',// 查询表格列头信息 前缀 例如:purchasesupplierArt Fm/itemFm/mdataFm
        requestBaseUrl: "/stock/boxLibLost",
        uniqueId: "blId"
      },
      // 查询参数 ,注意格式
      queryParamsDefault: [
  ...
  methods: {
    tableRowClick(rowData, rowIndex) {
      ...
      if (rowData != null) {
        ...
        this.currrentRowItem.rowName =
          rowData.biNo + " " + rowData.stationName;
      }
    ...
    getItemDataById() {
      //debugger
      let url = `/stock/boxLibLost/item/list`;
      let data = {
        blId: this.masterRowSelection.id
      };
      let _self = this;
