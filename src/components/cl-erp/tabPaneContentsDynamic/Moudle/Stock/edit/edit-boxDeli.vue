                   ...
                   <td :key="subIndex" :class="`ivu-table-column-${column.align}`" v-if="excludeFiled('itemFm',column.key)" :width="column.editWidth">
                     <!-- 控件特殊处理 工单号 -->
                      <template v-if="column.key =='workNo'">
                        <Input v-model="row[column.key]"  :disabled="column.readOnly"  @input="value => {valueChangeAssign(value, index, row,column.key)}" >
...
export default {
 ...
 created() {
  ...
  methods: {
          ...
          switch (obj.event) {  
            case 'biNewPriceChangeEvent':
                let currentItem = this.formDataInfo['boxDeliItems'].defaultList[obj.index]
                 let crrentPrice =0
                 if(currentItem.biNewPrice==null || currentItem.biNewPrice=='' || currentItem.biNewPrice=='0'){
                   crrentPrice =Number(currentItem.biPrice)
                 }else{
                   crrentPrice =Number(currentItem.biNewPrice)
                 }
                let totalMoney =crrentPrice * Number(currentItem.biQty==null?0:currentItem.biQty)
                  // 触发修改值 直接修改原始数据 无效
    ...
    // 设置默认值
    setDefaultData(){
        this.formDataInfo.master.bdDate = new Date(this.formDataInfo.master.bdDate) 
        this.formDataInfo.master.bdMonthDate = new Date(this.formDataInfo.master.bdMonthDate) 
    ...
    slave_list_table_edit_Delete(index){
      let tt = this.formDataInfo['boxDeliItems'].defaultList
