            ...
            <Col span="4">
              ...
              <Col span="6">
                <FormItem  :label-width="10">
                   <Checkbox @on-change="getDataByMode()" class="CheckboxItem" v-model="formDataInfo.master.storeFlag">加上没有库存的记录</Checkbox>
                   <Checkbox class="CheckboxItem" v-model="formDataInfo.master.coType">送货数为库存数</Checkbox>
                </FormItem>
            </Col>
             <Col span="1">
                 <FormItem>
             ...
             <template  v-for="(column,subIndex) in initColData.columns[`${functionParams.formInitPreName}Fm`].editColumns">
                   <td :key="subIndex" :class="`ivu-table-column-${column.align}`" v-if="excludeFiled('Fm',column.key)" :width="column.editWidth">
                     ...
                     <template  v-else-if="column.key ==='deliQty'">
                      <formControl  @input="tableColTiggerEvent(row,column,index,'deliQtyChangeEvent')"  :control-type="column.controlType" v-model="row[column.key]"></formControl>
                    </template>
                       <!--其它 控件 -->
                     <template v-else>
  ...
  methods: {
      // 表格列值改变 回调事件处理event：{row, column, index, event,config}
        tableColTiggerEventCall (obj) {
          switch (obj.event) {  
            case 'deliQtyChangeEvent':
                let currentItem = this.formDataInfo['boxDeliSlaveItems'].defaultList[obj.index]
                let deliQty =Number(currentItem.deliQty) 
                let bsQty = Number(currentItem.bsQty)
                if(deliQty>bsQty){
                  deliQty = bsQty
                }
               this.setSubListDeliQty(deliQty)
              break 
            default:
              break
          }
        ...
        tableColTiggerEventSubCall(obj){
            switch (obj.event) {  
            case 'deliQtyChangeEvent':
                let currentItem = this.formDataInfo['boxDeliSubItems'].defaultList[obj.index]
                let deliQty =Number(currentItem.deliQty) 
                let bsQty = Number(currentItem.bsQty)
                if(deliQty>0 && deliQty>bsQty){
                    this.$Modal.error({
                      width:"260",
                      title: '错误',
                      content: `送货数应小于或等于库存数`,
                      onOk: () => {
                        this.$refs['sub_list_table_edit'].set({ deliQty: bsQty}, obj.index)
                      }
                })
                }else if(deliQty==0){
