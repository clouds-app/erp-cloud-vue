 ...
 methods: {
   //计算盘点数量  qty
    ...
    onFill(index) {
      ...
      //获取工单号
      let workNo = this.formDataInfo.boxUseAdjustItemSlave.defaultList[index].workNo;
      //获取当前子表数据
      ...
      if (workNo) {
          request
            .post(`/stock/boxUseAdjust/getWorkInStore`, {whId:this.formDataInfo.master.paStationId,flag:1})
            .then(res => {
            let data = res[0];
            ...
            //         JSON.stringify(this.initData.initData.stockBoxUseAdjustitemFm)
            //       );
            //       return;
            //     }
            //   }
      ...
      for (let index1 = 0; index1 < one.length; index1++) {
        if (index1 !== index) {
          if (one[index1].workNo===workNo) {
            ...
            // this.$set(
              this.formDataInfo.boxUseAdjustItemSlave.defaultList[index]=[]
            //   index,
            //   this.initData.initData.stockBoxUseAdjustitemFm
            // )
    ...
    closeActionTigger() {
      ...
      this.formDataInfo.boxUseAdjustItemSlave.defaultList=Array[this.initData.initData.stockBoxUseAdjustitemFm]
    },
    //主表弹框判空
    clickValuedate() {
      //debugger;
