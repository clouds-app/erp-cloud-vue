    ...
    <div :style="{ height: tableHeight + 'px' }" v-if="formInitData.listView">
      <Row>
          <vTable
            ...
            name="stockBoxUseAdjustFm"
          ></vTable>
        </Row>
      <Row>
          <Tabs>    
            <TabPane label="用料库存调整明细表" name="name1">
              <vTable
...
export default {
  ...
  components: {
    editForm: function(resolve) {
      //组件的异步加载
      require(["./edit/edit-boxUseAdjust"], resolve);
    },
    htmlTemplate,
    vTable
  },
    ...
    tableRowClick(rowData, rowIndex) {
      ...
      if (rowData != null) {
        ...
        this.currrentRowItem.rowName =
          rowData.paStationCode + " " + rowData.paStationName;
      }
