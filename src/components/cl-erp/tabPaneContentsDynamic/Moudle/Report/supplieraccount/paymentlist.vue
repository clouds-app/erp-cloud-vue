      ...
      <Tabs @on-click="TabsClickEvent" :animated="false">
        <TabPane label="明细" name="detailed">
          <vTable
            ...
            :columns-url="functionParams.requestColBaseUrl + '/answerpayreportFm'"
            :table-data="tableBoxCoWorkProcData"
            :pagination="false"
            name="answerpayreportFm"
          ></vTable>
        </TabPane>
        ...
        <TabPane label="汇总" name="Summary">
          <vTable
            ...
            ref="tableFieldRef"
            :table-data="tableBoxCoWorkProcData"
            :columns-url="functionParams.requestColBaseUrl + '/answerpaySumreportFm'"
            :pagination="false"
            name="answerpaySumreportFm"
          ></vTable>
        </TabPane>
        <ReportPage 
            slot="extra" 
...
export default {
  ...
  data() {
    return {
      ...
      // 查询配置参数
      functionParams: {
        formInitPreName: "answerpayreportFm", // 查询表格列头信息 前缀 例如:boxco mainFm/itemFm/mdataFm
  ...
  methods: {
    ...
    LoadData(url) {
        ...
        let params = {
            ...
            apMonth:!!this.formDataInfo.apMonth ? dayjs(this.formDataInfo.apMonth).format('YYYYMM'):'',// 会计月份,
