      ...
      <Tabs @on-click="TabsClickEvent" :animated="false">
        <TabPane label="明细" name="detailed">
          <vTable
            ...
            :columns-url="functionParams.requestColBaseUrl + '/AccountPurInitInfoFm'"
            :table-data="tableBoxCoWorkProcData"
            :pagination="false"
            name="AccountPurInitInfoFm"
          ></vTable>
        </TabPane>
        ...
        <TabPane label="汇总" name="Summary">
          <vTable
            ...
            ref="tableFieldRef"
            :table-data="tableBoxCoWorkProcData"
            :columns-url="functionParams.requestColBaseUrl + '/AccountPurInitSumFm'"
            :pagination="false"
            name="AccountPurInitSumFm"
          ></vTable>
        </TabPane>
        <ReportPage 
            slot="extra" 
...
export default {
  ...
  data() {
    return {
      ...
      // 查询配置参数
      functionParams: {
        formInitPreName: "AccountPurInitInfoFm", // 查询表格列头信息 前缀 例如:boxco mainFm/itemFm/mdataFm
